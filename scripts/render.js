...
$(document).ready( function () {
    ...
    /* initial cookies at enter site first */
    cookies_init(); 
...
{
    let last_card_id = $('.card-box').last().attr('id').split('-')[1],
...
/* materialize components initial */
function materialize_init() 
{
    ...
    /* initial fixed button */
    $('.fixed-action-btn').floatingActionButton({
        direction: 'top',
        hoverEnabled: false
    });

    /* initial cards name tooltips */
    $('.tooltipped').tooltip({
        enterDelay: 1000
    });
}

/* helper function: replace string */
